...
`--reviewer-patterns` を指定すると、特定レビュワーの思考の癖に特化したパタンランゲージを抽出するため、「パタン候補の見つけ方」と「採用基準」を切り替えます。このフラグを使う入力JSONLは全レコードの `author` が同じである必要があります。異なるauthorが混じる場合は、先に `review-patterns filter --author <login>` で絞り込んでください。
