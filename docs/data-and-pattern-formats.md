...
- `--reviewer-patterns` 指定時は、入力JSONLの全レコードが同じ `author` を持つことを検証し、特定レビュワーが繰り返し注目する兆候、判断軸、フォースの重みづけを中心に候補化する。
...
- パタン候補の採用、保留、棄却の判断基準。
- レビュワー特化モードでは、通常のリポジトリ横断パタンではなく、対象レビュワーの安定した観察癖やトレードオフ判断を抽象化するための候補化・採用基準。
