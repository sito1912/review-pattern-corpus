...
- 指定されたJSONLコーパスファイルを読む。
- レコードをPull Request、review thread、返信関係ごとに再構成し、レビュー議論の文脈を読む。
- 挨拶、単なる賛同、対応完了だけの返信を主証拠にせず、指摘の受理、誤解、スコープ外判断を知る補助証拠として扱う。
- 個々の指摘を「文脈、表面上の指摘、背後の問題、フォース、解決の核、証拠」に分解する。
- ファイル名や単語一致ではなく、同じ問題とフォースを共有するレビュー群としてクラスタリングする。
- パタン候補の採用、保留、棄却の基準を明示する。
- 採用したパタンを既定のMarkdown見出しと `catalog.yaml` に沿って書く。
- 新しいレビュー例と既存パタンを比較する。
...
- 書き出し前に、候補が単なるチェックリスト、好み、過剰一般化、生コメント保存になっていないか自己レビューする。

MVPでは、プロンプトテンプレートはCLIに組み込みます。`--mode auto` の場合、`--patterns-dir` に `.md`、`.yaml`、`.yml` ファイルがなければ初回抽出用、あれば差分更新用のプロンプトを生成します。

生成プロンプトには以下を含めます。

- 守るべき抽象化・プライバシー方針。
- JSONLの読み方と、レビュー議論からパタン候補を抽出する思考プロセス。
- パタン候補の採用、保留、棄却の判断基準。
- パタンMarkdownと `catalog.yaml` の書き方。
- 初回抽出または差分更新での判断基準。
- 書き出し前の自己レビュー観点。
- 既存パタンファイルのパス一覧。差分更新時のみ。既存パタン本文はプロンプト本文に埋め込まない。
- レコード数、コメント種別、主な対象パスなどのコーパス概要。
- 今回のレビューJSONLファイルパス。JSONL本文はプロンプト本文に埋め込まない。
